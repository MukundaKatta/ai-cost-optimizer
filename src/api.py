"""FastAPI application for ai-cost-optimizer."""
...
app = FastAPI(title="ai-cost-optimizer", version="0.1.0", description="ai-cost-optimizer API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "ai-cost-optimizer", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "ai-cost-optimizer", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "ai-cost-optimizer", "version": "0.1.0", "ready": True,
